...
func (s Sandbox) ListBackups(destRel string) ([]Entry, error) {
	abs, err := s.Resolve(destRel)
	if err != nil {
		return nil, err
	}
	...
	for _, d := range ds {
		...
		out = append(out, Entry{Name: d.Name(), Path: filepath.ToSlash(filepath.Join(destRel, d.Name())), Type: "file", Size: info.Size(), Modified: info.ModTime()})
	}
	return out, nil
}
